...
static void speculatePHINodeLoads(IRBuilderTy &IRB, PHINode &PN) {
  ...
  // TODO: If any of the loads has a freeze then the inserted loads should too.

  // Rewrite all loads of the PN to use the new PHI.
  bool hasFreezingLoad {false};
  while (!PN.use_empty()) {
    LoadInst *LI = cast<LoadInst>(PN.user_back());
    hasFreezingLoad = hasFreezingLoad || loadHasFreeze(LI);
...
static void speculateSelectInstLoads(SelectInst &SI, LoadInst &LI,
  ...
  LoadInst *TL = IRB.CreateAlignedLoad(
      ...
      LI.getName() + ".sroa.speculate.load.true", loadHasFreeze(&LI));
  LoadInst *FL = IRB.CreateAlignedLoad(
      LI.getType(), FV, LI.getAlign(),
      LI.getName() + ".sroa.speculate.load.false", loadHasFreeze(&LI));
...
private:
  ...
  Value *rewriteVectorizedLoadInst(LoadInst &LI) {
    ...
    LoadInst *Load =
        IRB.CreateAlignedLoad(NewAI.getAllocatedType(), &NewAI,
                              NewAI.getAlign(), "load", loadHasFreeze(&LI));

    Load->copyMetadata(LI,
                       {LLVMContext::MD_mem_parallel_loop_access,
                        LLVMContext::MD_access_group, LLVMContext::MD_freeze});
  ...
  bool visitLoadInst(LoadInst &LI) {
    ...
    if (VecTy) {
      ...
      LoadInst *NewLI = IRB.CreateAlignedLoad(
          ...
          LI.isVolatile(), LI.getName(), loadHasFreeze(&LI));
      if (AATags)
        NewLI->setAAMetadata(AATags.adjustForAccess(
            NewBeginOffset - BeginOffset, NewLI->getType(), DL));
    ...
    if (IsSplit) {
      ...
      Value *Placeholder =
          new LoadInst(LI.getType(), PoisonValue::get(IRB.getPtrTy(AS)), "",
                       false, Align(1));
      V = insertInteger(DL, IRB, IRB.CreateFreeze(Placeholder),
                        IRB.CreateFreeze(V), NewBeginOffset - BeginOffset,
                        "insert");
      LI.replaceAllUsesWith(V);
  ...
  bool rewriteVectorizedStoreInst(Value *V, StoreInst &SI, Value *OldOp,
    ...
    if (V->getType() != VecTy) {
      ...
      // TODO: This load is created natively and is used to immediatly generate
      // a vector. I believe the application of freeze is not necessary from
      // basic example testing, but further tests are required.
  ...
  bool visitMemTransferInst(MemTransferInst &II) {
    ...
    if (VecTy && !IsWholeAlloca && !IsDest) {
      Src = IRB.CreateAlignedLoad(NewAI.getAllocatedType(), &NewAI,
                                  NewAI.getAlign(), "load", /* freeze */ true);
...
private:
  ...
  struct LoadOpSplitter : public OpSplitter<LoadOpSplitter> {
    ...
    LoadOpSplitter(Instruction *InsertionPoint, Value *Ptr, Type *BaseTy,
                   ...
                   IRBuilderTy &IRB, bool IsFreezing)
        : OpSplitter<LoadOpSplitter>(InsertionPoint, Ptr, BaseTy, BaseAlign, DL,
                                     IRB),
          AATags(AATags), IsFreezing(IsFreezing) {}
  ...
  for (LoadInst *LI : Loads) {
    ...
    for (;;) {
      ...
      LoadInst *PLoad = IRB.CreateAlignedLoad(
          ...
          /*IsVolatile*/ false, LI->getName(), loadHasFreeze(LI));
...
public:
  ...
  Value *getValueToUseForAlloca(Instruction *I) const override {
    return PoisonValue::get(ZeroType);
  }
