...
; CHECK-NEXT:    [[DOTFR1:%.*]] = freeze float [[TMP1]]
; CHECK-NEXT:    [[TMP2:%.*]] = load float, ptr [[ARRAYIDX2]], align 4
; CHECK-NEXT:    [[DOTFR:%.*]] = freeze float [[TMP2]]
; CHECK-NEXT:    [[CMP_I:%.*]] = fcmp fast olt float [[DOTFR1]], [[DOTFR]]
; CHECK-NEXT:    [[DOTV:%.*]] = select i1 [[CMP_I]], float [[DOTFR]], float [[DOTFR1]]
...
; CHECK-NEXT:    [[LD1_FR:%.*]] = freeze float [[LD1]]
; CHECK-NEXT:    [[LD2:%.*]] = load float, ptr [[LOADADDR2:%.*]], align 4
; CHECK-NEXT:    [[LD2_FR:%.*]] = freeze float [[LD2]]
; CHECK-NEXT:    [[COND:%.*]] = fcmp ogt float [[LD1_FR]], [[LD2_FR]]
; CHECK-NEXT:    [[LD_V:%.*]] = select i1 [[COND]], float [[LD1_FR]], float [[LD2_FR]]
