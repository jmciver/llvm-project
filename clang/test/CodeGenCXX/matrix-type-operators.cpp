...
void test_add_template() {
  ...
  // NOOPT:       [[MAT1:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !noundef [[NOUNDEF:![0-9]+]]{{$}}
  // NOOPT:       [[MAT2:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:         [[MAT1:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF:![0-9]+]]{{$}}
  // OPT:         [[MAT2:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
void test_subtract_template() {
  ...
  // NOOPT:       [[MAT1:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !noundef [[NOUNDEF]]{{$}}
  // NOOPT:       [[MAT2:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:         [[MAT1:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // OPT:         [[MAT2:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
void test_DoubleWrapper1_Sub1(MyMatrix<double, 10, 9> &m) {
  ...
  // NOOPT:       [[MATRIX:%.*]] = load <90 x double>, ptr {{.*}}, align 8, !noundef [[NOUNDEF]]{{$}}
  // OPT:         [[MATRIX:%.*]] = load <90 x double>, ptr {{.*}}, align 8, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
void test_insert_template1(MyMatrix<unsigned, 2, 2> &Mat, unsigned e, unsigned i, unsigned j) {
  ...
  // NOOPT:         [[MAT_ADDR:%.*]] = load ptr, ptr %Mat.addr, align 8, !noundef [[NOUNDEF:![0-9]+]]{{$}}
  // NOOPT-NEXT:    [[E:%.*]] = load i32, ptr %e.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // NOOPT-NEXT:    [[I:%.*]] = load i32, ptr %i.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // NOOPT-NEXT:    [[J:%.*]] = load i32, ptr %j.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[MAT_ADDR:%.*]] = load ptr, ptr %Mat.addr, align 8, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:      [[E:%.*]] = load i32, ptr %e.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:      [[I:%.*]] = load i32, ptr %i.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:      [[J:%.*]] = load i32, ptr %j.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  ...
  // NOOPT:         [[E:%.*]] = load i32, ptr %e.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // NOOPT:         [[I:%.*]] = load i32, ptr %i.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[E:%.*]] = load i32, ptr %e.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[I:%.*]] = load i32, ptr %i.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
void test_insert_template2(MyMatrix<float, 3, 8> &Mat, float e) {
  ...
  // NOOPT:         [[MAT_ADDR:%.*]] = load ptr, ptr %Mat.addr, align 8, !noundef [[NOUNDEF]]{{$}}
  // NOOPT-NEXT:    [[E:%.*]] = load float, ptr %e.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[MAT_ADDR:%.*]] = load ptr, ptr %Mat.addr, align 8, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:      [[E:%.*]] = load float, ptr %e.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  ...
  // NOOPT:         [[E:%.*]] = load float, ptr %e.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // NOOPT:         [[I:%.*]] = load i32, ptr %i.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[E:%.*]] = load float, ptr %e.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[I:%.*]] = load i32, ptr %i.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // CHECK-NEXT:    [[I_EXT:%.*]] = zext i32 [[I]] to i64
  // NOOPT-NEXT:    [[J:%.*]] = load i32, ptr %j.addr, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:      [[J:%.*]] = load i32, ptr %j.addr, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
int test_extract_template(MyMatrix<int, 2, 2> Mat1) {
  ...
  // NOOPT:         [[MAT:%.*]] = load <4 x i32>, ptr {{.*}}, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[MAT:%.*]] = load <4 x i32>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
double test_matrix_subscript(double4x4 m) {
  ...
  // NOOPT:         [[MAT:%.*]] = load <16 x double>, ptr {{.*}}, align 8, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[MAT:%.*]] = load <16 x double>, ptr {{.*}}, align 8, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  ...
  // NOOPT-NEXT:    [[RES:%.*]] = load double, ptr [[CALL]], align 8, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:      [[RES:%.*]] = load double, ptr [[CALL]], align 8, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
const double &test_matrix_subscript_reference(const double4x4 m) {
  ...
  // NOOPT:         [[NAMELESS1:%.*]] = load <16 x double>, ptr [[M_ADDR]], align 8, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[NAMELESS1:%.*]] = load <16 x double>, ptr [[M_ADDR]], align 8, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
double extract_IntWrapper_idx(double4x4 &m, IntWrapper i, UnsignedWrapper j) {
  ...
  // NOOPT-NEXT:    [[MAT_ADDR:%.*]] = load ptr, ptr %m.addr, align 8, !noundef [[NOUNDEF]]{{$}}
  // NOOPT-NEXT:    [[MAT:%.*]] = load <16 x double>, ptr [[MAT_ADDR]], align 8, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:      [[CMP:%.*]] = icmp ult i64 [[IDX2]], 16
  // OPT-NEXT:      call void @llvm.assume(i1 [[CMP]])
  // OPT-NEXT:      [[MAT_ADDR:%.*]] = load ptr, ptr %m.addr, align 8, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:      [[MAT:%.*]] = load <16 x double>, ptr [[MAT_ADDR]], align 8, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
void test_constexpr1(matrix_type<float, 4, 4> &m) {
  ...
  // NOOPT:         [[MAT:%.*]] = load <16 x float>, ptr {{.*}}, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[MAT:%.*]] = load <16 x float>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  ...
  // NOOPT-NEXT:   [[I:%.*]] = load i32, ptr %i, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:     [[I:%.*]] = load i32, ptr %i, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
  // CHECK-NEXT:   [[I_EXT:%.*]] = zext i32 [[I]] to i64
  // NOOPT-NEXT:   [[I2:%.*]] = load i32, ptr %i, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT-NEXT:     [[I2:%.*]] = load i32, ptr %i, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
...
void test_constexpr2(matrix_type<int, 5, 5> &m) {
  ...
  // NOOPT:         [[MAT:%.*]] = load <25 x i32>, ptr {{.*}}, align 4, !noundef [[NOUNDEF]]{{$}}
  // OPT:           [[MAT:%.*]] = load <25 x i32>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !noundef [[NOUNDEF]]{{$}}
