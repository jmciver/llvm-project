...
void test_add_template() {
  ...
  // NOOPT:       [[MAT1:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !freeze !{{[0-9]+}}{{$}}
  // NOOPT:       [[MAT2:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT:         [[MAT1:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  // OPT:         [[MAT2:%.*]] = load <10 x float>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
...
void test_DoubleWrapper1_Sub1(MyMatrix<double, 10, 9> &m) {
  ...
  // NOOPT:       [[MATRIX:%.*]] = load <90 x double>, ptr {{.*}}, align 8, !freeze !{{[0-9]+}}{{$}}
  // OPT:         [[MATRIX:%.*]] = load <90 x double>, ptr {{.*}}, align 8, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
...
void test_insert_template1(MyMatrix<unsigned, 2, 2> &Mat, unsigned e, unsigned i, unsigned j) {
  ...
  // NOOPT-NEXT:    [[E:%.*]] = load i32, ptr %e.addr, align 4, !freeze !{{[0-9]+}}{{$}}
  // NOOPT-NEXT:    [[I:%.*]] = load i32, ptr %i.addr, align 4, !freeze !{{[0-9]+}}{{$}}
  // NOOPT-NEXT:    [[J:%.*]] = load i32, ptr %j.addr, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT:           [[MAT_ADDR:%.*]] = load ptr, ptr %Mat.addr, align 8, !tbaa !{{[0-9]+}},
  // OPT-NEXT:      [[E:%.*]] = load i32, ptr %e.addr, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  // OPT-NEXT:      [[I:%.*]] = load i32, ptr %i.addr, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  ...
  // NOOPT:         [[E:%.*]] = load i32, ptr %e.addr, align 4, !freeze !{{[0-9]+}}{{$}}
  // NOOPT:         [[I:%.*]] = load i32, ptr %i.addr, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT:           [[E:%.*]] = load i32, ptr %e.addr, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  ...
  // CHECK-NEXT:    [[MAT:%.*]] = load <4 x i32>, ptr {{.*}}, align 4, !freeze !{{[0-9]+}}{{$}}
...
void test_insert_template2(MyMatrix<float, 3, 8> &Mat, float e) {
  ...
  // NOOPT:         [[MAT_ADDR:%.*]] = load ptr, ptr %Mat.addr, align 8,
  // NOOPT-NEXT:    [[E:%.*]] = load float, ptr %e.addr, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT:           [[MAT_ADDR:%.*]] = load ptr, ptr %Mat.addr, align 8, !tbaa !{{[0-9]+}},
  // OPT-NEXT:      [[E:%.*]] = load float, ptr %e.addr, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  ...
  // OPT-NEXT:      call void @llvm.assume(i1 [[CMP]])
  // CHECK-NEXT:    [[MAT:%.*]] = load <24 x float>, ptr {{.*}}, align 4, !freeze !{{[0-9]+}}{{$}}
...
int test_extract_template(MyMatrix<int, 2, 2> Mat1) {
  ...
  // NOOPT:         [[MAT:%.*]] = load <4 x i32>, ptr {{.*}}, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT:           [[MAT:%.*]] = load <4 x i32>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
...
double test_matrix_subscript(double4x4 m) {
  ...
  // NOOPT:         [[MAT:%.*]] = load <16 x double>, ptr {{.*}}, align 8, !freeze !{{[0-9]+}}{{$}}
  // OPT:           [[MAT:%.*]] = load <16 x double>, ptr {{.*}}, align 8, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  ...
  // NOOPT-NEXT:    [[RES:%.*]] = load double, ptr [[CALL]], align 8, !freeze !{{[0-9]+}}{{$}}
  // OPT-NEXT:      [[RES:%.*]] = load double, ptr [[CALL]], align 8, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
...
const double &test_matrix_subscript_reference(const double4x4 m) {
  ...
  // NOOPT:         [[NAMELESS1:%.*]] = load <16 x double>, ptr [[M_ADDR]], align 8, !freeze !{{[0-9]+}}{{$}}
  // OPT:           [[NAMELESS1:%.*]] = load <16 x double>, ptr [[M_ADDR]], align 8, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
...
double extract_IntWrapper_idx(double4x4 &m, IntWrapper i, UnsignedWrapper j) {
  ...
  // NOOPT-NEXT:    [[MAT:%.*]] = load <16 x double>, ptr [[MAT_ADDR]], align 8, !freeze !{{[0-9]+}}{{$}}
  // OPT-NEXT:      [[CMP:%.*]] = icmp ult i64 [[IDX2]], 16
  // OPT-NEXT:      call void @llvm.assume(i1 [[CMP]])
  // OPT-NEXT:      [[MAT_ADDR:%.*]] = load ptr, ptr %m.addr, align 8, !tbaa !{{[0-9]+}},
  // OPT-NEXT:      [[MAT:%.*]] = load <16 x double>, ptr [[MAT_ADDR]], align 8, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
...
void test_constexpr1(matrix_type<float, 4, 4> &m) {
  ...
  // NOOPT:         [[MAT:%.*]] = load <16 x float>, ptr {{.*}}, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT:           [[MAT:%.*]] = load <16 x float>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  ...
  // NOOPT-NEXT:   [[I:%.*]] = load i32, ptr %i, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT-NEXT:     [[I:%.*]] = load i32, ptr %i, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  // CHECK-NEXT:   [[I_EXT:%.*]] = zext i32 [[I]] to i64
  // NOOPT-NEXT:   [[I2:%.*]] = load i32, ptr %i, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT-NEXT:     [[I2:%.*]] = load i32, ptr %i, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  // CHECK-NEXT:   [[I2_EXT:%.*]] = zext i32 [[I2]] to i64
  // CHECK-NEXT:   [[IDX1:%.*]] = mul i64 [[I2_EXT]], 4
  // CHECK-NEXT:   [[IDX2:%.*]] = add i64 [[IDX1]], [[I_EXT]]
  // OPT-NEXT:     [[CMP:%.*]] = icmp ult i64 [[IDX2]], 16
  // OPT-NEXT:     call void @llvm.assume(i1 [[CMP]])
  // CHECK-NEXT:   [[MAT:%.*]] = load <16 x float>, ptr %result, align 4, !freeze !{{[0-9]+}}{{$}}
...
void test_constexpr2(matrix_type<int, 5, 5> &m) {
  ...
  // NOOPT:         [[MAT:%.*]] = load <25 x i32>, ptr {{.*}}, align 4, !freeze !{{[0-9]+}}{{$}}
  // OPT:           [[MAT:%.*]] = load <25 x i32>, ptr {{.*}}, align 4, !tbaa !{{[0-9]+}}, !freeze !{{[0-9]+}}{{$}}
  ...
  // OPT-NEXT:     call void @llvm.assume(i1 [[CMP]])
  // CHECK-NEXT:   [[MAT:%.*]] = load <25 x i32>, ptr %result, align 4, !freeze !{{[0-9]+}}{{$}}
