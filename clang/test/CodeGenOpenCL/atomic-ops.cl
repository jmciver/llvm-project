...
void fi1(atomic_int *i) {
  // CHECK-LABEL: @fi1
  // CHECK: load atomic i32, ptr %{{[.0-9A-Z_a-z]+}} syncscope("workgroup") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  int x = __opencl_atomic_load(i, memory_order_seq_cst, memory_scope_work_group);

  // CHECK: load atomic i32, ptr %{{[.0-9A-Z_a-z]+}} syncscope("agent") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  x = __opencl_atomic_load(i, memory_order_seq_cst, memory_scope_device);

  // CHECK: load atomic i32, ptr %{{[.0-9A-Z_a-z]+}} seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  x = __opencl_atomic_load(i, memory_order_seq_cst, memory_scope_all_svm_devices);

  // CHECK: load atomic i32, ptr %{{[.0-9A-Z_a-z]+}} syncscope("wavefront") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
...
void fi5(atomic_int *i, int scope) {
  ...
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("workgroup") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: br label %[[continue:.*]]
  // CHECK: [[opencl_device]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("agent") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: br label %[[continue]]
  // CHECK: [[opencl_allsvmdevices]]:
  // CHECK: load atomic i32, ptr %{{.*}} seq_cst, align 4
  // CHECK: br label %[[continue]]
  // CHECK: [[opencl_subgroup]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("wavefront") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: br label %[[continue]]
  // CHECK: [[continue]]:
...
void fi6(atomic_int *i, int order, int scope) {
  ...
  // CHECK-NEXT: ]
  // CHECK: [[MON_WG]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("workgroup-one-as") monotonic, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[MON_DEV]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("agent-one-as") monotonic, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[MON_ALL]]:
  // CHECK: load atomic i32, ptr %{{.*}} monotonic, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[MON_SUB]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("wavefront-one-as") monotonic, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[ACQ_WG]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("workgroup-one-as") acquire, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[ACQ_DEV]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("agent-one-as") acquire, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[ACQ_ALL]]:
  // CHECK: load atomic i32, ptr %{{.*}} acquire, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[ACQ_SUB]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("wavefront-one-as") acquire, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[SEQ_WG]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("workgroup") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[SEQ_DEV]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("agent") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[SEQ_ALL]]:
  // CHECK: load atomic i32, ptr %{{.*}} seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
  // CHECK: [[SEQ_SUB]]:
  // CHECK: load atomic i32, ptr %{{.*}} syncscope("wavefront") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
...
float ff1(global atomic_float *d) {
  // CHECK-LABEL: @ff1
  // CHECK: load atomic i32, ptr addrspace(1) {{.*}} syncscope("workgroup-one-as") monotonic, align 4, !freeze !{{[0-9]+}}{{$}}
...
int test_volatile(volatile atomic_int *i) {
  ...
  // CHECK-NEXT: %[[res:.*]] = load atomic volatile i32, ptr %[[addr]] syncscope("workgroup") seq_cst, align 4, !freeze !{{[0-9]+}}{{$}}
